...
func init() {
	android.RegisterModuleType("orion_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = orionExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = orionExpandVariables(ctx, dep_file)
	...
	cmd := orionExpandVariables(ctx, String(g.properties.Cmd))
